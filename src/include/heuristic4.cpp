...
namespace Heur4 {
    void construction(const Problem &p, const Caches &c, Solution& temp) {
        ...
        while (validCountAisles > 0) {
            ...
            int attempts = min(validCountAisles, SAMPLE_SIZE);
            
            for(int k = 0; k < attempts; ++k) {
                // Pick random index from valid range
                int randPos = std::uniform_int_distribution<>(0, validCountAisles - 1)(rng);
            ...
            if (validSamplePositions.empty()) {
                ...
                else chosenPos = std::uniform_int_distribution<>(0, validCountAisles - 1)(rng);
            ...
            std::swap(aisleCandidates[chosenPos], aisleCandidates[validCountAisles - 1]);
